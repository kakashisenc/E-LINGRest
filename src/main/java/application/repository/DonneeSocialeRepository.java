...
public class DonneeSocialeRepository {
    ...
    public List<DonneesSociales> findByCriteria(String nom){
        ...
        return em.createQuery("SELECT d from DonneesSociales d WHERE d.nom = '" + nom + "'").getResultList();

    }
    public List<DonneesSociales> findByCriteria(String nom, String prenom){

        return em.createQuery("SELECT d from DonneesSociales d WHERE d.nom = :nom and d.prenom = :prenom").getResultList();
